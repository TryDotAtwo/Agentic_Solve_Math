...
### 9. First manual end-to-end submit (for a human operator)

When a human operator has Kaggle credentials locally configured and a `submission.csv` built by any solver, the recommended first manual test looks like this:

1. **Prepare `submission.csv` in the project root** of `CayleyPy_444_Cube/` with at least a few rows and the columns:
   - `id` — integers with no gaps/duplicates in this toy file;
   - `solution` — arbitrary strings or simple move sequences (they may be incorrect as solutions; here we only test the pipeline).
2. **Run the local schema validation in a Python REPL or script**:
   - import `load_submission_csv` / `validate_submission_schema` from `CayleyPy_444_Cube.submission`;
   - call them on `Path("submission.csv")`;
   - fix the file until validation passes without exceptions.
3. **Run the autosubmit module via CLI** from the project root:
   - make sure the `kaggle` CLI works for you on some test command (e.g. `kaggle competitions list`);
   - run:
     - `python -m CayleyPy_444_Cube.submission.autosubmit --file submission.csv --message "444-cube pipeline smoke test"`  
       (optionally adding `--competition` if you want to override the default slug).
4. **Verify on Kaggle** that the submission appears on the competition page and that:
   - the file is accepted (no schema errors on Kaggle’s side);
   - you see a score or at least a \"Pending\" status.

This one manual test establishes that:

- the local code and docs agree on the submission format;
- your `kaggle.json` / CLI setup is correct;
- the 444-cube subproject can autonomously submit any future `submission.csv` generated by more advanced solvers.
